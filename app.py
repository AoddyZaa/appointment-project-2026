...
# --- ฟังก์ชันเชื่อมต่อ Google Sheets และ Google Calendar ผ่าน Streamlit Secrets ---
@st.cache_resource
...
try:
    ...
    st.error(
        f"⚠️ เชื่อมต่อ Google ไม่สำเร็จ: กรุณาตรวจสอบการตั้งค่าใน Secrets บน Streamlit Cloud ({e})"
    )
    st.stop()
...
st.write(
    "กรอกข้อมูลด้านล่าง ระบบจะบันทึกลง Google Sheets และ **ยิงเข้า Google Calendar พร้อมตั้งเตือนอัตโนมัติ** ให้ทันที!"
)
st.markdown("---")
...
if submitted:
    if not title_val:
        ...
        try:
            ...
            st.success(
                "🎉 บันทึกข้อมูลลง Google Sheets และส่งเข้า Google Calendar พร้อมตั้งเตือนอัตโนมัติสำเร็จเรียบร้อยแล้วครับ!"
            )

        except Exception as e:
            st.error(
                f"❌ เกิดข้อผิดพลาดระหว่างบันทึกข้อมูล: {e} (โปรดตรวจสอบอีเมล Service Account ว่าได้แชร์สิทธิ์เข้าปฏิทินหรือยัง)"
            )
